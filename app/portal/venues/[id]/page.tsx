                ...
                <div className="portal-surface rounded-3xl border p-4 sm:p-6">
                  ...
                  <div className="mt-5 space-y-4">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-black/18 p-3">
                      ...
                      <p className="mt-1 text-xs leading-5 text-white/62">Paste a venue Instagram profile or post/reel link. Use the note to explain why it matters, e.g. &quot;New happy hour menu posted this week.&quot; Manual updates only; no API, scraping, or feed embeds.</p>
                      <div className="mt-3 grid gap-3 md:grid-cols-2">
                        ...
                        <div className="md:col-span-2"><label className="mb-1 block text-sm font-medium text-white/85">Social note</label><input type="text" value={venueForm.social_note} onChange={(event) => updateVenueForm('social_note', event.target.value)} placeholder="New happy hour menu posted this week; Tonight's DJ lineup posted; Fresh food special announced" className="min-h-[44px] w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-white outline-none focus:border-orange-300/40" /></div>
                      </div>
                    </div>
                ...
                <div className="portal-surface rounded-3xl border p-4 sm:p-6">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-white/55">
                        {scheduleType === 'sport'
                          ...
                          : 'Choose one edit type, then work in the focused form below.'}
                      </div>
                      <div className="mt-3">
                  ...
                  {isEventScheduleType(scheduleType) ? (
                    <div className="mt-4 grid gap-3 md:mt-5 md:grid-cols-2">
                      <div><label className="mb-1 block text-sm font-medium text-white/82">{scheduleType === 'sport' ? 'Fixture / event title' : 'Title'}</label><input type="text" value={title} onChange={(event) => setTitle(event.target.value)} placeholder={scheduleType === 'sport' ? 'e.g. State of Origin Game 1' : scheduleType === 'daily_special' ? 'e.g. Steak Night' : scheduleType === 'lunch_special' ? 'e.g. Lunch Special' : 'Optional event title'} className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-white outline-none focus:border-orange-300/40" /></div>
                      <div><label className="mb-1 block text-sm font-medium text-white/82">{scheduleType === 'sport' ? 'Sport type' : isEventScheduleType(scheduleType) ? 'Summary' : 'Deal text / summary'}</label><input type="text" value={dealText} onChange={(event) => setDealText(event.target.value)} placeholder={scheduleType === 'sport' ? 'e.g. Rugby League, AFL, UFC, Football' : scheduleType === 'daily_special' ? 'e.g. Parmi + chips $20' : scheduleType === 'lunch_special' ? 'e.g. Lunch special $15' : 'Short event summary for the public card'} className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-white outline-none focus:border-orange-300/40" /></div>
                    </div>
                  ) : null}
                  {isVenueRuleScheduleType(scheduleType) ? (
                  ...
                  {(!isDealScheduleType(scheduleType) && (isEventScheduleType(scheduleType) || scheduleType === 'happy_hour')) ? (
                    <div className="mt-3.5"><label className="mb-1 block text-sm font-medium text-white/82">Description</label><textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={3} placeholder={scheduleType === 'sport' ? 'Optional fixture detail, venue screen area, or broadcast context' : isDealScheduleType(scheduleType) ? 'Optional detail for this special or offer' : isEventScheduleType(scheduleType) ? 'Optional event detail for the public card or venue page' : 'Optional happy hour detail if you need more context'} className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-white outline-none focus:border-orange-300/40" /></div>
                  ) : null}
                  {(!isDealScheduleType(scheduleType) && (isEventScheduleType(scheduleType) || isVenueRuleScheduleType(scheduleType) || scheduleType === 'happy_hour')) ? (
                    <div className="mt-3.5"><label className="mb-1 block text-sm font-medium text-white/82">Notes</label><textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={3} placeholder={isVenueRuleScheduleType(scheduleType) ? 'Optional nuance such as Beer garden only or Front bar only' : scheduleType === 'sport' ? 'Optional notes such as Live & loud, main bar only, or sound for major games' : 'Optional notes'} className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-white outline-none focus:border-orange-300/40" /></div>
                  ) : null}
                  {scheduleType === 'happy_hour' ? (
                    <div className="mt-5 space-y-4">
